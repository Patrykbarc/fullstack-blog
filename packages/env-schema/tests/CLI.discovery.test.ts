...
describe('CLI recursive discovery', () => {
	let root: string;

	beforeEach(() => {
		root = mkdtempSync(join(tmpdir(), 'env-schema-test-'));
	});
	...
	it('skips node_modules and hidden directories', async () => {
		...
		// Bogus .env deep inside node_modules — must not generate output
	...
	it('skips .env files in directories without src/ (e.g. repo root)', async () => {
		// Root-level .env without src/ — must be skipped
